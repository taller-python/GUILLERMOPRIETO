...
class Consultar(Empleado):
     def consultado(self, valor_Cedula):
          ...
          if   (nmb_longitud_valor_Cedula >= 1)  : 
            try:
                cursor = cole_cliente.find(({"Cedula":{"$in":[valor_Cedula]}}))
                for emp in cursor:
...
#Consultar_Empleado = Consultar("71700800")
#print(Consultar_Empleado.consultado("71700800"))
#Consultar_Empleado = Consultar("8600700")
#print(Consultar_Empleado.consultado("8600700"))
Consultar_Empleado = Consultar("8400500")
print(Consultar_Empleado.consultado("8400500"))
#cursor = cole_cliente.find(({"Cedula":{"$in":["43700800"]}}))
#try:
#    cursor = cole_cliente.find(({"Cedula":{"$in":[cedula_empleado]}}))
#    for emp in cursor:
#        print( "%s  - %s- %s - %i - %i - %i"  %(emp['Cedula'], emp['Cargo'], emp['Correo'], emp['ValorHora'], emp['HorasTrabajadas'], emp['SalarioaPagar']))
